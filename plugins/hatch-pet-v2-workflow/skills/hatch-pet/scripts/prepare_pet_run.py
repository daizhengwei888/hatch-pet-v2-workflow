...
# Modified from OpenAI's Apache-2.0 hatch-pet skill; see repository NOTICE.
...
REACTION_PRESETS = {
    "jump": {
        "label": "playful jump",
        "semantic": "jumping",
        "action": STATE_PROMPTS["jumping"],
        "requirements": STATE_REQUIREMENTS["jumping"],
    },
    "heart-cute": {
        "label": "hand-heart with a gentle wink",
        "semantic": "reaction",
        "action": (
            "Native pointer reaction in exactly five stages: prepare, raise both hands, "
            "form a clear heart with both hands at the chest, hold with a gentle head tilt "
            "and wink, then return to the starting pose."
        ),
        "requirements": [
            "The hand-heart must be formed by the hands or remain physically joined to the hands and chest; never draw a detached or floating heart.",
            "Keep both feet and the body baseline planted; this is a gesture, not a jump.",
            "Use a warm, friendly expression and keep hair, hats, glasses, and clothing attached and identity-consistent.",
        ],
    },
    "cute": {
        "label": "hands-to-cheeks cute pose",
        "semantic": "reaction",
        "action": (
            "Native pointer reaction in exactly five stages: prepare, lift both hands to "
            "the cheeks, tilt the head slightly, hold a blink and soft smile, then return."
        ),
        "requirements": [
            "Keep the gesture readable through the face, head, and hands without detached symbols or effects.",
            "Keep both feet and the body baseline planted; this is a gesture, not a jump.",
            "Preserve all identity cues, accessories, hair, and clothing across the five frames.",
        ],
    },
    "cool": {
        "label": "cap-and-glasses cool pose",
        "semantic": "reaction",
        "action": (
            "Native pointer reaction in exactly five stages: prepare, touch or tip the hat "
            "brim, lightly adjust the glasses while turning the upper body, hold a confident "
            "smile, then return to the starting pose."
        ),
        "requirements": [
            "If the character has no hat or glasses, translate the same cool beat into one existing identity-safe accessory or a confident side pose; do not invent a prop.",
            "Keep hats and glasses rigidly registered to the head with no accessory drift.",
            "Keep both feet and the body baseline planted; this is a gesture, not a jump.",
        ],
    },
    "cheer": {
        "label": "encouraging cheer",
        "semantic": "reaction",
        "action": (
            "Native pointer reaction in exactly five stages: prepare, gather energy with "
            "small clenched hands, lift both hands in a compact cheer, hold a bright smile, "
            "then return."
        ),
        "requirements": [
            "Express celebration with the body and hands only; no confetti, text, stars, or detached effects.",
            "Keep the body centered and the feet near the baseline; do not turn this into a locomotion jump.",
            "Preserve identity, costume, props, and proportions across every frame.",
        ],
    },
    "surprise": {
        ...
        "semantic": "reaction",
        "action": (
            "Native pointer reaction in exactly five stages: prepare, widen the eyes, bring "
            "both hands to the cheeks, hold a shy playful smile, then return."
        ),
        "requirements": [
            "Express surprise through the face, head, and hands only; no punctuation, symbols, or detached effects.",
            "Keep both feet and the body baseline planted; this is a gesture, not a jump.",
            "Preserve identity, costume, props, and proportions across every frame.",
        ],
    },
...
def state_contract(
    args: argparse.Namespace, state: str
) -> tuple[str, list[str], str, str]:
    if state != "jumping" or args.interaction_preset == "jump":
        return (
            STATE_PROMPTS[state],
    ...
    preset = REACTION_PRESETS[args.interaction_preset]
    return (
        "Atlas state and row id remain `jumping`; semanticOverride maps QA and generation semantics to `reaction`. "
        + str(preset["action"]),
        list(preset["requirements"]),
        "reaction",
        "Keep apparent pet scale and the planted baseline stable across all five frames; the native `jumping` row is being used as a non-jumping pointer reaction.",
    )


def standard_row_request(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for state, row, frames, purpose in ROWS:
        ...
        if state == "jumping" and args.interaction_preset != "jump":
            entry["purpose"] = REACTION_PRESETS[args.interaction_preset]["label"]
            entry["interactionPreset"] = args.interaction_preset
            entry["semanticOverride"] = {"jumping": "reaction"}
        rows.append(entry)
    return rows
...
def row_prompt(args: argparse.Namespace, state: str, row: int, frames: int, purpose: str) -> str:
    ...
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    state_prompt, requirements, semantic, continuity = state_contract(args, state)
...
Style: {style_contract}
Animation continuity: {continuity} Move the pose within the slot instead of redrawing the pet larger or smaller frame to frame.

State action ({semantic}): {state_prompt}
...
Use the attached canonical base and layout guide for exact identity, style, scale, baseline, face construction, materials, palette, markings, props, and spacing. Read `qa/look-mechanics.md` and use the pet's natural gaze mechanism.
...
def make_jobs(
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        if state == "jumping":
            job["semantic_role"] = (
                "jumping" if args.interaction_preset == "jump" else "reaction"
            )
            job["interaction_preset"] = args.interaction_preset
            if args.interaction_preset != "jump":
                job["semanticOverride"] = {"jumping": "reaction"}
        jobs.append(job)
    ...
    for state, row, directions, _purpose in LOOK_ROWS:
        depends_on = ["look-cardinals", *standard_job_ids]
        if row == 10:
            depends_on.append("look-row-9")
        continuity_inputs = (
            []
            if row == 9
            else [
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["none", "native-pointer"],
        help="Native event target. Reaction presets use the existing jumping row and do not add unsupported pet.json fields.",
    )
    parser.add_argument(
        "--interaction-preset",
        default="jump",
        choices=sorted(REACTION_PRESETS),
        help="Semantic content for row 4. Default jump preserves legacy behavior.",
    )
    parser.add_argument(
        "--chroma-key",
        default="auto",
    ...
    args.interaction_preset = args.interaction_preset.strip().lower()
    if args.interaction_preset != "jump":
        args.interaction_target = "native-pointer"
    ...
    request = {
        ...
        "atlas": ATLAS,
        "rows": standard_row_request(args)
        + [
            {
                "state": state,
                "row": row,
        ...
        "interaction": {
            "target": args.interaction_target,
            "preset": args.interaction_preset,
    ...
    jobs = {
        ...
        "run_dir": str(run_dir),
        "primary_generation_skill": "$imagegen",
        "jobs": make_jobs(args, run_dir, copied_refs),
    }
