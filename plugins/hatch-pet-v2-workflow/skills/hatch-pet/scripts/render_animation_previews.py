...
def load_atlas_frames(atlas: Image.Image, row: int, expected_count: int) -> list[Image.Image]:
    return [
        atlas.crop(
            (
                column * CELL_WIDTH,
...
def main() -> None:
    ...
    if args.atlas:
        ...
        if atlas.size != (CELL_WIDTH * 8, CELL_HEIGHT * 11):
            raise SystemExit(f"atlas must be 1536x2288, found {atlas.size[0]}x{atlas.size[1]}")
    output_dir = Path(args.output_dir).expanduser().resolve()
    previews = []
    for row, (state, durations) in enumerate(ROW_DURATIONS.items()):
        frames = (
            load_atlas_frames(atlas, row, len(durations))
            if atlas is not None
            else load_frames(frames_root, state, len(durations))
        )
