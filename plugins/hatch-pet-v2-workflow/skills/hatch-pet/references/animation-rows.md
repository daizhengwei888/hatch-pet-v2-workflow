...
- `jumping`: without a semantic override, anticipation, lift, peak, descent, and settle. With `semanticOverride.jumping: reaction`, keep the same row, five frames, and timing but validate the configured native-pointer reaction instead. See `reaction-presets.md`.
