...
├── pet.json
└── <manifest-selected-spritesheet>.webp
```
...
`spritesheetPath` may use any safe relative WebP or PNG path that stays inside
the pet directory. Prefer a content-addressed filename when installing or
upgrading a pet so Codex cannot reuse an older cached atlas under the same path.
...
Run-only fields such as `interaction`, `semanticOverride`, family identity, retry state, and QA evidence never belong in `pet.json`. Native Codex still addresses row 4 as `jumping`; a configured reaction changes only the five cells' visual meaning.
