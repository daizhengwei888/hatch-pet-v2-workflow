...
- Row 4 defaults to jump semantics. When `pet_request.json` contains `interaction.semanticOverride.jumping == reaction`, validate the named reaction preset as preparation, readable peak gesture, and return; do not require vertical jump motion.
- A `heart-cute` reaction forms or attaches the heart to the hands/chest. Detached or floating hearts fail.
...
- Reaction first/fifth frames connect naturally to idle, worn accessories remain registered, and the action is readable at `192x208` without text.
